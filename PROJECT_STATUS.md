...
*   **知识图谱可视化 (`/graph`)**：目前后端已有图谱搜索接口，前端需要引入 `React Flow` 或 `Cytoscape.js` 来真正渲染出实体节点和关系边的可视化网络图。
*   **文件详情与预览 (`/files/[fileId]`)**：需要实现文件详情页面，支持直接在网页内预览 PDF，并高亮显示提取出的分片。
*   **报告编辑器 (`/reports`)**：实现前端页面以查看、编辑和导出生成的 Markdown 报告。
